...
def main():
    args = get_args()
    set_seed(args.seed)
    datadir = "/mnt/efs/people/wuahmad/datasets/python_runtime_errors"

    train_dataset = load_dataset("json", data_files=os.path.join(datadir, "train_balanced.json"))
    valid_dataset = load_dataset("json", data_files=os.path.join(datadir, "valid_balanced.json"))
    test_dataset = load_dataset("json", data_files=os.path.join(datadir, "test_balanced.json"))
    print("Done with data loading", train_dataset.keys())
    ...
    labels = ClassLabel(num_classes=29, names=error_label_list)

    def convert_examples_to_features(example):
        inputs = tokenizer(example["code"], truncation=True, max_length=args.max_seq_length)
        label = labels.str2int(example["target_label"])
        return {
